    ...
    <div className="h-full flex flex-col gap-6">
      <div className="flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Practice previously asked questions from top companies.</p>
        </div>
      ...
      <div className="flex-1 flex gap-6 min-h-0 pb-2">
        <Card className="w-80 flex flex-col p-4 bg-card shrink-0 shadow-sm border-border/80">
          <h3 className="font-semibold text-slate-500 mb-4 px-2 uppercase text-xs tracking-wider">Top Companies</h3>
          <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar pr-2">
            ...
            {filteredCompanies.map((company) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-3 rounded-xl transition-all duration-200 text-left ${activeCompany?.name === company.name ? 'bg-primary/10 border border-primary/20 text-primary shadow-sm' : 'hover:bg-[#f5f0eb] border border-transparent text-slate-500 hover:border-[#e2ddd8]'}`}
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${activeCompany?.name === company.name ? 'bg-primary/20 text-primary' : 'bg-[#f5f0eb] text-slate-500'}`}>
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-3">
              <select
                ...
                className="bg-background border border-border text-sm rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary/50 text-slate-500 cursor-pointer shadow-sm"
              >
              ...
              <div key={q._id} className="p-5 rounded-xl border border-border bg-background hover:border-slate-600 transition-all duration-200 flex items-center justify-between group cursor-pointer shadow-sm hover:shadow-md">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${q.type === "MCQ" ? "bg-blue-500/10 text-blue-400 border border-blue-500/20" : "bg-purple-50 text-purple-600 border border-purple-500/20"}`}>
                      {q.type}
                    </span>
                  ...
                  <div className="flex gap-3 mt-3 text-sm items-center">
                    <span className={`px-2.5 py-0.5 rounded text-xs font-semibold ${q.difficulty === 'Easy' ? 'text-green-600 bg-green-400/10 border border-green-500/20' : q.difficulty === 'Medium' ? 'text-yellow-400 bg-yellow-400/10 border border-yellow-500/20' : 'text-red-600 bg-red-400/10 border border-red-500/20'}`}>
                      {q.difficulty}
                    </span>
                    <span className="w-1 h-1 rounded-full bg-[#e2ddd8]"></span>
                    <span className="text-slate-500 bg-[#f5f0eb]/80 px-2.5 py-0.5 rounded text-xs border border-[#e2ddd8] font-medium">{q.topic}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {q.type === "MCQ" ? (
                    <Button
                      variant="secondary"
                      className="gap-1.5 opacity-80 group-hover:opacity-100 transition-all duration-200 shadow-sm hover:border-primary/40 hover:text-primary hover:bg-[#1e3a5f]/5"
