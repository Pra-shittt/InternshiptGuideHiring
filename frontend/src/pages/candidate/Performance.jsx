...
export function Performance() {
  ...
  if (!data) {
    return (
      <div className="h-full flex items-center justify-center">
        <p className="text-slate-400">No data available. Take a mock test first!</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col gap-6 pb-2">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Detailed breakdown of your test performance and topic mastery</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 shrink-0">
        ...
        <Card className="p-5 border-l-4 border-l-indigo-500 shadow-sm">
          <h3 className="text-sm font-medium text-slate-400 uppercase tracking-wider">Total Tests</h3>
        ...
        <Card className="p-5 border-l-4 border-l-amber-500 shadow-sm">
          <h3 className="text-sm font-medium text-slate-400 uppercase tracking-wider">Topics Covered</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 min-h-0">
            ...
            {data.topicStats?.map((t) => (
              <div key={t.topic} className="p-4 rounded-xl border border-border bg-background">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={`text-sm font-semibold ${t.accuracy >= 70 ? 'text-green-400' : t.accuracy >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>{t.accuracy}%</span>
                </div>
        ...
        <div className="flex flex-col gap-6">
          <Card className="p-6 flex-1 shadow-sm">
            <h2 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2"><TrendingDown className="w-5 h-5 text-red-400" /> Weak Areas</h2>
          ...
          <Card className="p-6 flex-1 shadow-sm">
            <h2 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-green-400" /> Strong Areas</h2>
            <div className="space-y-2">
              ...
              {data.strongTopics?.map((t) => (
                <div key={t.topic} className="flex items-center justify-between p-3 rounded-lg border border-green-500/20 bg-green-500/5">
                  <span className="font-medium text-foreground">{t.topic}</span>
                  <span className="text-green-400 text-sm font-semibold">{t.accuracy}%</span>
                </div>
              ))}
            </div>
          </Card>
