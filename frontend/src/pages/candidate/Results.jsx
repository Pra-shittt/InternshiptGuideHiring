    ...
    <div className="h-full flex flex-col gap-6 pb-2">
      <div className="flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Review your mock test history and scores.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 shrink-0">
        ...
        <Card className="p-5 border-l-4 border-l-indigo-500 shadow-sm bg-card/60 backdrop-blur-sm">
          <h3 className="text-sm font-medium text-slate-500 uppercase tracking-wider">Tests Taken</h3>
        ...
        <Card className="p-5 border-l-4 border-l-amber-500 shadow-sm bg-card/60 backdrop-blur-sm">
          <h3 className="text-sm font-medium text-slate-500 uppercase tracking-wider">Best Score</h3>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar border border-border rounded-xl bg-background/50 shadow-inner">
            <table className="w-full text-sm text-left border-collapse">
              ...
              <tbody className="divide-y divide-border text-slate-500 bg-card/40">
                ...
                {attempts.map((a, idx) => (
                  <tr key={a.testSessionId} className="hover:bg-[#f5f0eb] transition-colors">
                    ...
                    <td className="px-6 py-4 text-green-600">{a.correctAnswers}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded text-xs font-semibold ${a.percentage >= 70 ? 'text-green-600 bg-green-400/10 border border-green-500/20' : a.percentage >= 50 ? 'text-yellow-400 bg-yellow-400/10 border border-yellow-500/20' : 'text-red-600 bg-red-400/10 border border-red-500/20'}`}>
                        {a.percentage}%
                      </span>
                    </td>
