    ...
    <div className="h-full flex flex-col gap-6 pb-2">
      <div className="flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Top performers ranked by total score</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-3 gap-4 shrink-0">
          {[leaders[1], leaders[0], leaders[2]].map((l, i) => (
            <Card key={l.userId} className={`p-6 text-center ${i === 1 ? 'border-yellow-500/30 bg-yellow-500/5 scale-105' : ''} shadow-sm`}>
              ...
              <p className="text-sm text-slate-400 mt-1">{l.accuracy}% accuracy • {l.testCount} tests</p>
            </Card>
          ))}
        </div>
      )}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar border border-border rounded-xl bg-background/50 shadow-inner">
          <table className="w-full text-sm text-left border-collapse">
            ...
            <tbody className="divide-y divide-border text-slate-300 bg-card/40">
              ...
              {leaders.map((l) => (
                <tr key={l.userId} className="hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded text-xs font-semibold ${l.accuracy >= 70 ? 'text-green-400 bg-green-400/10' : l.accuracy >= 50 ? 'text-yellow-400 bg-yellow-400/10' : 'text-red-400 bg-red-400/10'}`}>{l.accuracy}%</span>
                  </td>
                </tr>
              ))}
            </tbody>
